...
double PathDepOption::PriceByMC(BSModel Model, long N,
                                        double epsilon)
{
    double H = 0.0, Hsq = 0.0, Heps = 0.0, HepsD = 0.0;
   SamplePath S(m);
   for(long i=0; i<N; i++)
   {
	  ...
	  Rescale(S, 1.0 - epsilon);          //rescale back to S
	  Rescale(S, 1.0 - epsilon);          //rescale to S*(1-epsilon)
	  HepsD = (i * HepsD + Payoff(S)) / (i + 1.0);

   }
   Price = exp(-Model.r*T)*H;
   PricingError = exp(-Model.r*T)*sqrt(Hsq-H*H)/sqrt(N-1.0);
   delta = exp(-Model.r*T)*(Heps-H)/(Model.S0*epsilon);
   gamma = exp(-Model.r * T) * (Heps - 2 * H + HepsD) / std::pow(Model.S0 * epsilon, 2);
   return Price;
}
